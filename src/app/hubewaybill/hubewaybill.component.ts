...
@Component({
  selector: 'app-hubewaybill',
  templateUrl: './hubewaybill.component.html',
  styleUrls: ['./hubewaybill.component.css'],
  providers: [AppGlobals]
})

export class HubewaybillComponent implements OnInit {
...
public model: any = { date: { year: 2019, month: 10, day: 9 } };
  ...
  getwaybilldetails(){
    var self = this;
    axios.get(`http://${this._global.baseUrl}:${this._global.port}/${this._global.version_no}/secure/waybills/details`)
        .then(function (res) {
          
          console.log(res);
          for(let key in res.data){
            if( res.data[key].waybill.van){
              res.data[key].waybillvanname = res.data[key].waybill.van.name;
            }
             
            if( res.data[key].waybill.route){
              res.data[key].waybillroutename = res.data[key].waybill.route.name;
            }
            
              res.data[key].waybilldate = res.data[key].waybill.date;
                var d = new Date(res.data[key].waybill.date); 
                var date = d.getUTCDate();
                var month = d.getUTCMonth() + 1;
                var year = d.getUTCFullYear();
                res.data[key].date = date + ' -' + month+ ' -' +year;
              if( res.data[key].product){
                res.data[key].productname = res.data[key].product.name;
              }
                
              if(res.data[key].waybill.route){
                if(res.data[key].waybill.route.hubsid == self.hubsid){
                  self.ebills.push(res.data[key]);
                }
              }
  ...
  getwaybills(){
    var self = this;
    axios.get(`http://${self._global.baseUrl}:${self._global.port}/${self._global.version_no}/secure/waybills/profiles?waybills[status]=1&routes[hubsid]=${self.hubsid}`)
        .then(function (res) {
          self.loading = false;
          console.log(res);
          for(let key in res.data){
  ...
  getmrp(x){
    ...
    axios.get(' http://103.214.233.141:2981/v1/secure/products/profiles/' + self.mrpprod[0].productsid)
        .then(function (res) {
          console.log(res);
        ...
        .then(function (res) {
          ...
          for(var i = 0; i < self.items; i++){
            console.log(self.selectedproduct[i][0]);
              axios.post('http://103.214.233.141:2981/v1/secure/waybills/details',{
  ...
  getwaybillprofilebyID(){
    var self = this;
    axios.get('http://103.214.233.141:2981/v1/secure/waybills/profiles/' +self.waybillsid)
        .then(function (res) {
          console.log(res);
  ...
  getallroutes(){
    var self = this;
    axios.get('http://103.214.233.141:2981/v1/secure/routes/profiles')
        .then(function (res) {
          console.log(res);
  ...
  getallvans(){
    var self = this;
    axios.get('http://103.214.233.141:2981/v1/secure/vans/profiles')
        .then(function (res) {
          console.log(res);
  ...
  getvanbyroute(){
    ...
    axios.get('http://103.214.233.141:2981/v1/secure/vans/profiles?routes[routesid]=' + self.selectedroute )
        .then(function (response) {
          console.log(response);
          for (let key in response.data){
            response.data[key].routename = response.data[key].route.name;
  ...
  getvanbyID(){
    ...
    console.log(self.selectedvan);
    if(self.selectedvan != ""){
    axios.get('http://103.214.233.141:2981/v1/secure/vans/profiles/' + self.selectedvan )
        .then(function (response) {
          console.log(response);
          self.flag = true;
           self.vanname = response.data.name;
           self.regno = response.data.name;
          self.routename = response.data.route.name;
        })
        .catch(function (error) {
          console.log(error);
        });
